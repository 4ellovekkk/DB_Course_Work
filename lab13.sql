...
GRANT CREATE TABLESPACE TO VYDRA_DBA;
GRANT ALTER TABLESPACE TO VYDRA_DBA;

create tablespace t1
    datafile 't1_VYDRA_DBA.dbf'
    size 7 m
    autoextend on
    maxsize unlimited
...
-- Тоже от SYS
alter user VYDRA_DBA quota unlimited on t1;
alter user VYDRA_DBA quota unlimited on t2;
alter user VYDRA_DBA quota unlimited on t3;
alter user VYDRA_DBA quota unlimited on t4;
...
select * from T_INTERVAL partition (SYS_P500);
...
select * from T_HASH partition(k2);
update T_HASH set str='qwerty' where id=3;
select * from T_HASH partition(k3);
