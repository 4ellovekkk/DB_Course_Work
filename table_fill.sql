...
BEGIN
    FOR i IN 1..100000
        LOOP
            -- Генерация случайных данных
            v_owner_id := TRUNC(DBMS_RANDOM.VALUE(179450, 200000)); -- Замените границы в зависимости от ваших требований
            v_is_locked := TRUNC(DBMS_RANDOM.VALUE(0, 2));
            v_account_type := TRUNC(DBMS_RANDOM.VALUE(0, 2));
            v_balance := DBMS_RANDOM.VALUE(0, 100000);
...
insert into ACTIONS(ACTION_NAME) values ('Refill');
insert into ACTIONS(ACTION_NAME) values ('Withdrawal');
insert into ACTIONS(ACTION_NAME) values ('Transfer');
...
BEGIN
    FOR i IN 1..10000 LOOP
        -- Генерация случайных данных
        v_account_id   := TRUNC(DBMS_RANDOM.VALUE(179450, 200000)); -- Замените границы в зависимости от ваших требований
        v_action_date  := TO_DATE('01-JAN-2023', 'DD-MON-YYYY') + DBMS_RANDOM.VALUE(1, 365); -- Замените границы в зависимости от ваших требований
        v_action_id    := TRUNC(DBMS_RANDOM.VALUE(2, 5)); -- Значения id действий из таблицы ACTIONS (2, 3, 4)
...
insert into CLERK_INFO(name, surname, thirdname, birth_date) VALUES ('Alexey','Makarov','Igorevich',to_date('2003-04-26','yyyy-mm-dd'));
insert into CLERK_LOGIN_PASSWORD (ID,LOGIN,PASSWORD) values (1,'lexa_login','123321');

insert into DEPOSIT_CONDITIONS(procentage_per_year, name, termin) VALUES (15,'Test deposit',3);